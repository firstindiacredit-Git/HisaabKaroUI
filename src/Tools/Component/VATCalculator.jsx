            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div className="space-y-4 sm:space-y-6">
                <form onSubmit={calculateVAT} className="space-y-4">
                  <div>
                    <label className="block text-sm dark:text-white font-medium text-gray-600 mb-2">
                      Select Country
                    </label>
                    <select
                      value={selectedCountry}
                      onChange={handleCountryChange}
                      className="w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 transition-colors"
                    >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <div className="relative">
                        <button
                          ...
                          onMouseEnter={() =>
                            setShowTooltip({ ...showTooltip, add: true })
                          }
                          onMouseLeave={() =>
                            setShowTooltip({ ...showTooltip, add: false })
                          }
                          className={`w-full px-4 py-3 rounded-lg font-medium transition-colors duration-200 ${
                            calculationType === "add"
                              ? "bg-indigo-600 text-white"
                              : "bg-white dark:text-white dark:bg-gray-700 text-gray-700 border border-gray-200 hover:bg-gray-50"
                          }`}
                        >
                          VAT Exclusive
                        </button>
                        {showTooltip.add && (
                          <div className="absolute z-10 w-64 p-2 mt-2 text-sm text-white bg-gray-900 rounded-lg shadow-lg">
                            When VAT is exclusive, the tax is added to the base
                            amount. Example: Base {currency}100 +{" "}
                            {countries[selectedCountry].vat}% VAT = Total{" "}
                            {currency}
                            {(
                              100 *
                              (1 + countries[selectedCountry].vat / 100)
                            ).toFixed(2)}
                          </div>
                        )}
                      </div>
                      <div className="relative">
                        <button
                          ...
                          onMouseEnter={() =>
                            setShowTooltip({ ...showTooltip, remove: true })
                          }
                          onMouseLeave={() =>
                            setShowTooltip({ ...showTooltip, remove: false })
                          }
                          className={`w-full px-4 py-3 rounded-lg font-medium transition-colors duration-200 ${
                        ...
                        {showTooltip.remove && (
                          <div className="absolute z-10 w-64 p-2 mt-2 text-sm text-white bg-gray-900 rounded-lg shadow-lg">
                            When VAT is inclusive, the tax is already included
                            in the total amount. Example: Total {currency}120
                            includes {countries[selectedCountry].vat}% VAT (Base{" "}
                            {currency}100)
                          </div>
                        )}
                      </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 transition-colors"
                      placeholder={`Enter ${
                        calculationType === "add" ? "base" : "total"
                      } amount`}
                      required
                    />
                  </div>
              ...
              <div className="space-y-4 sm:space-y-6">
                {result ? (
                  <>
                    <div className="bg-white dark:bg-gray-700 p-4 sm:p-6 rounded-xl border dark:border-gray-700 border-gray-200">
                      <div className="grid grid-cols-2 gap-3 sm:gap-4">
                        <div className="p-3 sm:p-4 dark:bg-gray-600 bg-indigo-50 rounded-lg sm:h-32 flex flex-col justify-center">
                          <p className="text-sm dark:text-white text-gray-600">Base Amount</p>
                          <p className="text-xl sm:text-2xl font-bold text-indigo-600 mt-2">
                            {formatCurrency(result.baseAmount)}
                          </p>
                        </div>
                          <div className="p-3 sm:p-4 dark:bg-gray-600 bg-green-50 rounded-lg sm:h-32 flex flex-col justify-center">
                          <p className="text-sm dark:text-white text-gray-600">VAT Amount</p>
                          <p className="text-xl sm:text-2xl font-bold text-green-600 mt-2">
                            {formatCurrency(result.vatAmount)}
                          </p>
                        </div>
                      </div>
                      <div className="mt-4 p-3 dark:bg-gray-600 sm:p-4 bg-purple-50 rounded-lg text-center">
                  ...
                  <div className="bg-white dark:bg-gray-700 p-6 rounded-xl border dark:border-gray-700 border-gray-200 h-full flex flex-col items-center justify-center text-gray-500">
                    <svg
